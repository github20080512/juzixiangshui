(function () {
  ...
  var isAuth = false;
  let PathName = path.resolve(__dirname, "../forcopy/record.json");
  let slideIndex = 0;
  ...
  function readFile() {
    var content = ipcRenderer.sendSync("readData", {
      readPath: path.resolve(__dirname, "../forcopy/configue.json"),
    });
    ...
    await fetch("http://bing.getlove.cn/latelyBingImageStory")
      ...
      .then(function (res) {
        ...
        lis[0].getElementsByTagName("img")[0].onload = function () {
          ...
          let audio = $("mp3");
          getclass("music-btn")[0].onmouseover = mp3Div.onmouseover =
            function () {
              clearTimeout(divHideTimer);
              mp3Div.style.display = "block";
            };
          getclass("music-btn")[0].onmouseleave = mp3Div.onmouseleave =
            function () {
          ...
          var musicFiles = readMusic(musicPath);
        ...
        getclass("like")[0].onclick = function () {
          ...
          writeInfoFile(
            path.resolve(__dirname, "../forcopy/configue.json"),
            JSON.stringify(info)
          );
...
<span class="s-remark">${tem[i]["remark"]}</span>
  ...
  for (let i = 0; i < navBtns.length; i++) {
    navBtns[i].onclick = function () {
      ...
      if (name == "diary") {
        renderDiary(getclass("diary-nav-animation")[0].innerText.trim());
      } else if (name == "bill") {
        renderBillIndex();
